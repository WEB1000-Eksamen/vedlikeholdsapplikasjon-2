<?php
    ...
    class User {
         ...
         */
        public function __construct ($RoomtypeName, $Beds) {
        ...
        public function create () {
            if (!$this->userExists()) {
                ...
                $Beds = $this->createHash($this->_Beds);
                $sql = "INSERT INTO users (RoomtypeName, Beds) VALUES (?, ?)";
                $stmt = $this->db->prepare($sql);

                $stmt->bind_param("ss", $this->_RoomtypeName, $Beds);
                $stmt->execute();
                $stmt->close();
         ...
         */
        public function changeBeds () {
            if ($this->userExists()) {
                $newPassword = $this->createHash($this->_Beds);
                $sql = "UPDATE users SET Password = ? WHERE Username = ?";
                $stmt = $this->db->prepare($sql);

                $stmt->bind_param("ss", $newPassword, $this->_RoomtypeName);
                $stmt->execute();
        ...
        private function userExists () {
            ...
            $stmt->bind_param("s", strtolower($this->_RoomtypeName));
            $stmt->execute();
            $stmt->store_result();
        ...
        private function setLogin () {
            ...
            $stmt->bind_param("s", $this->_RoomtypeName);
            $stmt->execute();
        }
        ...
        private function checkPassword () {
            $sql = "SELECT Beds FROM roomtypes WHERE Beds = ?";
            $stmt = $this->db->prepare($sql);
            ...
            if (password_verify($this->_Beds, $hash)) {
                return true;
            }
            return false;
        }
